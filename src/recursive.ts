...
export const RecursiveJS = map(fn => fn.length > 1 && fn.name !== 'equals' ? curry(fn) : fn, [
...
function adjust<T>(fn: (x: T) => T, index: number, xs: T[]): T[] {
  return (function adjust([x, ...xs]: T[], currentIndex:number = 0, acc: T[] = []): T[] {
